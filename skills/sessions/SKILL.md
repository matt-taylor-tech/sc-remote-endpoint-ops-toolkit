...
Config discovery (first match wins): `SC_URL`+`SC_AUTH_SECRET` env vars, `SC_CONFIG` env var (path to a config file), any mounted `*/mnt/Configs/screenconnect-config.json`, or `~/.config/screenconnect/screenconnect-config.json`. See the plugin README for the config format.
